# Orchid Agent
> agent 工作流协议（BOOTSTRAP / WORKER 与纪律），存于 **`.orchd/SKILL.md`**；使用手册见 [docs/user-manual.md](../docs/user-manual.md)。
...
**MUST NOT**：
1. **禁手动 git 写操作**：不得 `git checkout / branch / reset / stash / gc / prune / clean / rebase / cherry-pick / merge / push`；仅引擎自动执行，唯一豁免任务分支 `git commit`；手动先报告获许可。
